...
3. If the user named **Story IDs** (for example `S01.02.03`), open only the matching
   blocks in **`spec/stories.md`** and the related rows/sections in
   **`spec/task-breakdown.md`**.
4. For functional detail, pull **only** the **FR-/NFR-** sections from **`spec/prd.md`**
   (or the corresponding objects in **`spec/prd.spec.yaml`**) that those stories trace to.
5. If the user mentions recent changes, read the tail of **`spec/CHANGELOG.md`**.
6. Respect **`AGENTS.md`** and do not expand scope beyond the spec without explicit approval.
